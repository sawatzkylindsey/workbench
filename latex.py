...
"""
    ...
    def run(self, lines):
        ...
        for reg, math_mode, expr in tex_expr:
            m = hashlib.sha256()
            m.update(expr.encode("utf-8"))
            expr_key = m.hexdigest()
            #simp_expr = filter(str.isalnum, expr)
            import pdb
            #pdb.set_trace()
            if expr_key in self.cached:
                data = self.cached[expr_key]
            else:
                data = self._latex_to_base64(expr, math_mode)
                new_cache[expr_key] = data
            expr = expr.replace('"', "").replace("'", "")
            id += 1
            #pdb.set_trace()
            replacement = IMG_EXPR % (str(math_mode).lower(), expr, expr[:15] + "_" + str(id), str(data).replace("b'", "").replace("'", ""))
            #pdb.set_trace()
            page = reg.sub(replacement, page, 1)
